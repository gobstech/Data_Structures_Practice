...
No* aux_insere_qualquer_posicao(No *antigo, int valor, int posicao) {
  int cont = 1;
  No *novo, *aux=NULL, *aux1;
  novo = (No*) malloc(sizeof(No));
  novo->info=valor;
  novo->prox=NULL;
  aux1=antigo;

  while ((aux1 != NULL) && (cont < posicao)) {
    aux = aux1;
    aux1 = aux1->prox;
    cont++;
  }
...
void insere_qualquer_posicao(Lista* l, int valor, int posicao) {
  l->inicio = aux_insere_qualquer_posicao(l->inicio, valor, posicao);
}
...
int main() {
  ...
  insereL_fim(l, 10);
  insereL_fim(l, 9);
  insere_qualquer_posicao(l, 20, 1);
  imprimir_lista(l);
